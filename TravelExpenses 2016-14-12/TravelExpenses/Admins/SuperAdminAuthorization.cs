        ...
        private void refreshTravels()
        {
            ...
            if (title == "Executive Processor")
            {
                ...
                while (dr.Read())
                {
                    ...
                    double estTotal = Math.Round( Convert.ToDouble(dr["EstimatedTravelCost"].ToString()),2);
                    ...
                    string department = dr["Department"].ToString();
                    bool personalMileage = Convert.ToBoolean(dr["MileagePersonal"].ToString());
                    dgvAthorizations.Rows.Add(depDate.ToShortDateString(), retDate.ToShortDateString(), travEvent, travPurpose, budgeted, user, estTotal.ToString(), estimateID, userID, district, department, personalMileage);
                }
                dr.Close();
